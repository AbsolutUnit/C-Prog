...
Test(sfmm_basecode_suite, free_coalesce_flip, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    size_t sz_w = 8, sz_x = 200, sz_y = 300, sz_z = 4;
    /* void *w = */ sf_malloc(sz_w);
    void *x = sf_malloc(sz_x);
...
Test(sfmm_basecode_suite, dual_realloc, .timeout = TEST_TIMEOUT) {
    size_t sz_u = 200, sz_w = 200, sz_y = 200, sz_z = 700;
    void *u = sf_malloc(sz_u);
    void *w = sf_malloc(sz_w);
    void *y = sf_malloc(sz_y);

    void *l = sf_realloc(u, sz_z);
    void *k = sf_realloc(y, sz_u);
    sf_free(w);

    cr_assert(l != k, "Equivalent pointers");
    assert_free_block_count(0, 2);
    assert_free_block_count(416, 1);
    assert_free_block_count(6800, 1);
}

Test(sfmm_basecode_suite, two_page_iterative, .timeout = TEST_TIMEOUT) {
    void *x = sf_malloc(8128);
    void *y = sf_malloc(8192);
...
Test(sfmm_basecode_suite, mem_align_misalign, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    void *x = sf_memalign(64, 13);
    cr_assert_null(x, "X is not null!");
    cr_assert(sf_errno == EINVAL);
}

Test(sfmm_basecode_suite, mem_align_size, .timeout = TEST_TIMEOUT) {
    sf_errno = 0;
    void *x = sf_memalign(0, 32);
    printf("Here is %p", x);
    cr_assert_null(x, "X is not null!");
    cr_assert(sf_errno == 0);
}